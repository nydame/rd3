...
function menuRemoveEvent()
{	
	jQuery('.menu_size_remove_row').on( 'click', function(){
...
function ppbRemoveUnsaveStatus()
{
	...
	jQuery('.ppb_sortable li .shortcode_title').removeClass('edited');
	jQuery('.ppb_sortable li .shortcode_title').removeClass('duplicated');
...
1
	jQuery("#content_builder_sort li a.ppb_duplicate").on( 'click', function(){
		...
		//Display edit button
	    var editURL = tgAjax.ajaxurl+'?action=grandrestaurant_ppb&ppb_post_type='+postType+'&shortcode='+myCheckId+'&rel='+randomId+'&width=800&height=900&page_id='+jQuery('#ppb_page_id').val();
...
jQuery(document).ready(function(){
	...
	jQuery("#ppb_module_wrapper li").on( 'click', function(){
	    ...
	    if(myCheckId != '' && targetType == 'module')
	    {
			...
			//Display edit button
	    	var editURL = tgAjax.ajaxurl+'?action=grandrestaurant_ppb&ppb_post_type='+postType+'&shortcode='+myCheckId+'&rel='+randomId+'&width=800&height=900&page_id='+jQuery('#ppb_page_id').val()+'&builder_action=add';
	    	builderItem+= '<a data-rel="'+randomId+'" href="'+editURL+'" class="ppb_edit"><span class="dashicons dashicons-welcome-write-blog"></span>Edit</a>';
	...
	jQuery('#menu_size_sortable tbody').sortable({
	    helper: "clone",
	    placeholder: "sortable-placeholder",
	...
	jQuery('.pp_import_content_button').on( 'click', function(){
        ...
        //jQuery('#import_demo_content').hide();
	...
	jQuery('.pp_get_styling_button').on( 'click', function(){
        ...
        //jQuery('#get_styling_content').hide();
       
        var data = {
            'action': 'pp_get_styling',
	...
	jQuery('#menu_size_add_row').on( 'click', function(){
		var rowHTML = '';
		rowHTML+= '<tr>';
		rowHTML+= '<td class="sortable_handle"><span class="dashicons dashicons-menu"></span></td>';
		rowHTML+= '<td><input type="text" class="widefat" name="menu_size_item_title[]"></td>';
		rowHTML+= '<td><input type="text" class="widefat" name="menu_size_item_price[]"></td>';
		rowHTML+= '<td><a class="button menu_size_remove_row" href="javascript:;"><span class="dashicons dashicons-no-alt"></span></a></td>';
		rowHTML+= '</tr>';
		
		jQuery('#menu_size_sortable').find('tbody:last').append(rowHTML);
		menuRemoveEvent();
	});
	menuRemoveEvent();
});