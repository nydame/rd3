...
class WC_GC_Admin_Notices {
	...
	public static function update_notice() {
		...
		if ( WC_GC_Install::is_update_pending() ) {
			...
			$notice = __( '<strong>WooCommerce Gift Cards</strong> database update complete. Thank you for updating to the latest version!', 'woocommerce-gift-cards' );
	...
	/**
	 * Test the WC queueing system and display notice on failure.
	 * Rescheduled with every plugin update - see 'WC_GC_Install::install'.
	 *
	 * @since  1.3.2
	 */
	public static function queue_notice() {
		...
		$scheduled_date = WC()->queue()->get_next( 'woocommerce_gc_queue_test' );

		// Note that 'get_next' may return null if the task is being processed at the current request. At worst, this will re-schedule another test.
		if ( ! $scheduled_date ) {
			WC()->queue()->schedule_single( gmdate( 'U' ) + 60, 'woocommerce_gc_queue_test', array() );
			return;
		}

		// More than 1 day due?
		$show_notice = $scheduled_date && is_object( $scheduled_date ) && gmdate( 'U' ) - $scheduled_date->getTimestamp() > DAY_IN_SECONDS;

		if ( $show_notice ) {

			$notice = __( 'Gift Cards ran a quick check-up on your site, and found that the task scheduler built into WooCommerce might be failing to process certain tasks. To keep your site in top shape, please ask the host or administrator of your server to look into this for you.', 'woocommerce-gift-cards' );
