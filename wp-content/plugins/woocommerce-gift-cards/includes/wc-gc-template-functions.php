...
if ( ! function_exists( 'wc_gc_get_activity_description' ) ) {
	...
	function wc_gc_get_activity_description( $activity_data ) {
		...
		switch ( $activity_data->get_type() ) {
			case 'redeemed':
				/* translators: %1$s: Amount, %2$s: giftcard code */
				$description = sprintf( __( 'Added %1$s to your balance from gift card code %2$s', 'woocommerce-gift-cards' ), wc_price( $activity_data->get_amount() ), $mask ? wc_gc_mask_code( $activity_data->get_gc_code() ) : $activity_data->get_gc_code() );
				break;
			case 'refunded':
