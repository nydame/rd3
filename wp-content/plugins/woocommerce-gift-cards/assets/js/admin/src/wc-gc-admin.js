...
;( function( $, window ) {
	...
	$( function() {
		...
		function giftcard_checkbox_on_cb( on_load ) {
			...
			// Global Virtual.
			$virtual_checkbox.attr( 'checked', 'checked' ).change();
			...
			if ( $variable_product_options.length ) {
				$variable_product_options.find( '.variable_is_virtual' ).attr( 'checked', 'checked' ).each( function() {
					var $this = $( this );
					$this.change();
					$this.parent().hide();
				} );
		...
		function giftcard_checkbox_off_cb( on_load ) {
			...
			$virtual_checkbox.attr( 'checked', virtual_value_cache ? 'checked' : null ).change();
