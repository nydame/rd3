<?php
/*
Plugin Name: Grand Restaurant Theme Custom Post Type
Plugin URI: http://themes.themegoods2.com/grandrestaurant
Description: Plugin that will create custom post types for Grand Restaurant theme.
Version: 2.9
Author: ThemeGoods
...
define( 'PLUGINDOMAIN', 'grandrestaurant-custom-post' );

add_action('plugins_loaded', 'tg_load_textdomain');
function tg_load_textdomain() 
{
	load_plugin_textdomain( 'grandrestaurant-custom-post', FALSE, dirname( plugin_basename(__FILE__) ) . '/languages/' );
}
...
function post_type_menus() {
	$labels = array(
    	'name' => _x('Menus', 'post type general name', PLUGINDOMAIN),
    	'singular_name' => _x('Menu', 'post type singular name', PLUGINDOMAIN),
    	'add_new' => _x('Add New Menu', 'book', PLUGINDOMAIN),
    	'add_new_item' => __('Add New Menu', PLUGINDOMAIN),
    	'edit_item' => __('Edit Menu', PLUGINDOMAIN),
    	'new_item' => __('New Menu', PLUGINDOMAIN),
    	'view_item' => __('View Menu', PLUGINDOMAIN),
    	'search_items' => __('Search Menus', PLUGINDOMAIN),
    	'not_found' =>  __('No Menu found', PLUGINDOMAIN),
    	'not_found_in_trash' => __('No Menu found in Trash', PLUGINDOMAIN), 
    	'parent_item_colon' => ''
	);		
	$args = array(
    	'labels' => $labels,
	...
	register_post_type( 'menus', $args );
  	...
  	$labels = array(			  
  	  'name' => _x( 'Menu Categories', 'taxonomy general name', PLUGINDOMAIN ),
  	  'singular_name' => _x( 'Menu Category', 'taxonomy singular name', PLUGINDOMAIN ),
  	  'search_items' =>  __( 'Search Menu Categories', PLUGINDOMAIN ),
  	  'all_items' => __( 'All Menu Categories', PLUGINDOMAIN ),
  	  'parent_item' => __( 'Parent Menu Category', PLUGINDOMAIN ),
  	  'parent_item_colon' => __( 'Parent Menu Category:', PLUGINDOMAIN ),
  	  'edit_item' => __( 'Edit Menu Category', PLUGINDOMAIN ), 
  	  'update_item' => __( 'Update Menu Category', PLUGINDOMAIN ),
  	  'add_new_item' => __( 'Add New Menu Category', PLUGINDOMAIN ),
  	  'new_item_name' => __( 'New Menu Category Name', PLUGINDOMAIN ),
  	); 							  
...
$postmetas = 
	array (
		...
		'menus' => array(
		    ...
		    array("section" => "Menu Option", "id" => "menu_order_url", "type" => "text", "title" => "Order URL", "description" => "Enter URL to order this menu online (Optional)"),
		),
);
...
function new_meta_box() {
	...
	//Create menu size options
	if($_GET['post_type'] == 'menus')
	{
		echo "<strong>Size (Optional)</strong><hr class='pp_widget_hr'/>";
		echo "<div class='pp_widget_description'>You can create size options for this menu product ex. small, medium and large.</div><br/>";
		
		echo '<table id="menu_size_sortable">';
		
		echo '<thead>';
		echo '<tr>';
		
		echo '<th width="10%"></th>';
		echo '<th width="50%">Size Title</th>';
		echo '<th width="30%">Price</th>';
		echo '<th width="10%"></th>';
		...
		$menu_size_item_title_arr = get_post_meta($post->ID, 'menu_size_item_title', true);
		$menu_size_item_price_arr = get_post_meta($post->ID, 'menu_size_item_price', true);
		...
		if(!empty($menu_size_item_title_arr) && is_array($menu_size_item_title_arr))
		{
			foreach($menu_size_item_title_arr as $key => $menu_size_item_title_item)
			{
				if(isset($menu_size_item_price_arr[$key]))
				{
					...
					echo '<td><input type="text" class="widefat" name="menu_size_item_title[]" value="'.esc_attr($menu_size_item_title_item).'"></td>';
					echo '<td><input type="text" class="widefat" name="menu_size_item_price[]" value="'.esc_attr($menu_size_item_price_arr[$key]).'"></td>';
					echo '<td><a class="button menu_size_remove_row" href="javascript:;"><span class="dashicons dashicons-no-alt"></span></a></td>';
					echo '</tr>';
				}
			}
		}
		else
		{
			echo '<tr>';
			echo '<td class="sortable_handle"><span class="dashicons dashicons-menu"></span></td>';
			echo '<td><input type="text" class="widefat" name="menu_size_item_title[]"></td>';
			echo '<td><input type="text" class="widefat" name="menu_size_item_price[]"></td>';
			echo '<td><a class="button menu_size_remove_row" href="javascript:;"><span class="dashicons dashicons-no-alt"></span></a></td>';
			echo '</tr>';
		}
		...
		echo '<a id="menu_size_add_row" class="button" data-target="menu_size_sortable" href="javascript:;">Add another</a>';
...
function save_postdata( $post_id ) {
	...
	if (isset($_POST['pp_meta_form'])) 
	{
		...
		//Save custom post type custom fields
		...
		//Save menu size options
		if(isset($_POST['menu_size_item_title']) && isset($_POST['menu_size_item_price']))
		{
			update_custom_meta($post_id, $_POST['menu_size_item_title'], 'menu_size_item_title');
			update_custom_meta($post_id, $_POST['menu_size_item_price'], 'menu_size_item_price');
		}
		else
		{
			delete_post_meta($post_id, 'menu_size_item_title');
			delete_post_meta($post_id, 'menu_size_item_price');
		}
