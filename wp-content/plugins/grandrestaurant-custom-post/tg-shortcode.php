...
function tg_social_icons_func($atts, $content) {
	...
	{
		$return_html.='<li class="tripadvisor"><a target="_blank" title="Tripadvisor" href="'.$pp_tripadvisor_url.'"><i class="fa fa-tripadvisor"></i></a></li>';
	}
	
	$pp_yelp_url = get_option('pp_yelp_url');
	if(!empty($pp_yelp_url))
	{
		$return_html.='<li class="yelp"><a target="_blank" title="Yelp" href="'.$pp_yelp_url.'"><i class="fa fa-yelp"></i></a></li>';
	}
...
function tg_thumb_gallery_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		foreach($images_arr as $key => $image)
		{
			...
			$return_html.= '<div class="post_img small square_thumb shortcode" style="float:left;margin-right:10px;margin-bottom:10px">';
...
function tg_gallery_slider_func($atts, $content) {
	...
	wp_enqueue_script("flexslider-js", get_template_directory_uri()."/js/flexslider/jquery.flexslider-min.js", false, THEMEVERSION, true);
	wp_enqueue_script("script-gallery-flexslider", get_template_directory_uri()."/templates/script-gallery-flexslider.php?autoplay=".$autoplay.'&amp;caption='.$caption.'&amp;timer='.$timer.'&amp;height='.$height, false, THEMEVERSION, true);
	...
	if(!empty($images_arr))
	{
		...
		if(!empty($height))
		{
		    $return_html.= 'style="height:'.$height.'px;"';
		}
		
		$return_html.= '>';
		$return_html.= '<div class="flexslider tg_gallery" data-height="'.$height.'">';
		$return_html.= '<ul class="slides">';
